...
class Volcanoplot(Vis):
    ...
    def __init__(self, df: pd.DataFrame, log_fc: str, p_val: str, gene_column: str, invert=False, p_val_cutoff=0.05,
                 log_fc_cuttoff=2, label_big_sig=False, colours=None, offset=0, genes_to_label=None):
        super().__init__(df)
        self.log_fc = log_fc
        self.p_val = p_val
        self.p_val_cutoff = p_val_cutoff
        self.log_fc_cuttoff = log_fc_cuttoff
        self.genes_to_label = genes_to_label
    ...
    def add_scatter_and_annotate(self, fig: plt, x: np.array, y: np.array, colour: str, idxs: np.array):
        ax = fig.scatter(x[idxs], y[idxs], c=colour, alpha=self.opacity)
    ...
    def volcanoplot(self):
        ...
        self.add_scatter_and_annotate(fig, x, y, self.colours['ns_small-pos-logFC'], ns_small_pos_logFC)
        self.add_scatter_and_annotate(fig, x, y, self.colours['ns_big-pos-logFC'], ns_big_pos_logFC)
        self.add_scatter_and_annotate(fig, x, y, self.colours['sig_small-pos-logFC'], sig_small_pos_logFC)
        self.add_scatter_and_annotate(fig, x, y, self.colours['sig_big-pos-logFC'], sig_big_pos_logFC)
        # Check if we
        self.add_scatter_and_annotate(fig, x, y, self.colours['ns_small-neg-logFC'], ns_small_pos_logFC)
        self.add_scatter_and_annotate(fig, x, y, self.colours['ns_big-neg-logFC'], ns_big_pos_logFC)
        self.add_scatter_and_annotate(fig, x, y, self.colours['sig_small-neg-logFC'], sig_small_pos_logFC)
        self.add_scatter_and_annotate(fig, x, y, self.colours['sig_big-neg-logFC'], sig_big_pos_logFC)

        # Now plot labels on the points